...
function LoginPageClient() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setHomepagePassword((window as any).RUNTIME_CONFIG?.HOMEPAGE_PASSWORD || '');
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      // 静态导出下无 /api/login，直接在前端校验 homepagePassword 并设置 auth cookie
      if (homepagePassword && password !== homepagePassword) {
        setError('密码错误');
        return;
      }
      // 写入 auth cookie
      const encoded = encodeURIComponent(
        JSON.stringify({ role: 'user', password })
      );
