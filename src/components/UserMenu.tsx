...
export const UserMenu: React.FC = () => {
  ...
  const [isChangeSitePasswordOpen, setIsChangeSitePasswordOpen] = useState(false);
  ...
  const handleLogout = async () => {
    try {
      // 静态导出无 /api/logout，直接清除 cookie 即可
  ...
  const handleChangeSitePassword = () => {
    setIsOpen(false);
    setIsChangeSitePasswordOpen(true);
    setNewSitePassword('');
    setConfirmSitePassword('');
    setSitePasswordError('');
  };

  const handleCloseChangeSitePassword = () => {
    setIsChangeSitePasswordOpen(false);
    setNewSitePassword('');
    setConfirmSitePassword('');
    setSitePasswordError('');
  };
  ...
  const handleSubmitChangeSitePassword = async () => {
    setSitePasswordError('');
    if (!newSitePassword) {
      setSitePasswordError('新密码不得为空');
      return;
    }
    if (newSitePassword !== confirmSitePassword) {
      setSitePasswordError('两次输入的密码不一致');
      return;
    }
    ...
    try {
      const response = await fetch('/api/admin/password', {
        ...
        body: JSON.stringify({ newPassword: newSitePassword }),
      });
      const data = await response.json();
      if (!response.ok) {
        setSitePasswordError(data.error || '修改站点密码失败');
        return;
      }
      setIsChangeSitePasswordOpen(false);
      await handleLogout();
    } catch (error) {
      setSitePasswordError('网络错误，请稍后重试');
    } finally {
  ...
  // 检查是否显示修改密码按钮
  const showChangePassword = false; // 静态导出不提供个人密码修改

  const showChangeSitePassword = showAdminPanel && storageType !== 'localstorage';
    ...
    <>
      ...
      <div className='fixed top-14 right-4 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-xl z-[1001] border border-gray-200/50 dark:border-gray-700/50 overflow-hidden select-none'>
        ...
        <div className='py-1'>
          ...
          {/* 修改站点密码按钮（owner/admin） */}
          {showChangeSitePassword && (
            <button
              onClick={handleChangeSitePassword}
              className='w-full px-3 py-2 text-left flex items-center gap-2.5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-sm'
            >
              <KeyRound className='w-4 h-4 text-gray-500 dark:text-gray-400' />
              <span className='font-medium'>修改站点密码</span>
            </button>
          )}
  ...
  return (
    <>
      ...
      {/* 使用 Portal 将修改站点密码面板渲染到 document.body */}
      {isChangeSitePasswordOpen &&
        mounted &&
        createPortal(
          (
            <>
              <div
                className='fixed inset-0 bg-black/50 backdrop-blur-sm z-[1000]'
                onClick={handleCloseChangeSitePassword}
              />
              <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-gray-900 rounded-xl shadow-xl z-[1001] p-6'>
                <div className='flex items-center justify-between mb-6'>
                  <h3 className='text-xl font-bold text-gray-800 dark:text-gray-200'>修改站点密码</h3>
                  <button
                    onClick={handleCloseChangeSitePassword}
                    className='w-8 h-8 p-1 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors'
                    aria-label='Close'
                ...
                <div className='flex gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700'>
                  ...
                  <button
                    onClick={handleSubmitChangeSitePassword}
                    className='flex-1 px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-600 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
                    disabled={sitePasswordLoading || !newSitePassword || !confirmSitePassword}
                  >
                    {sitePasswordLoading ? '修改中...' : '确认修改'}
                  </button>
                </div>
                <div className='mt-4 pt-4 border-t border-gray-200 dark:border-gray-700'>
                  <p className='text-xs text-gray-500 dark:text-gray-400 text-center'>修改后需重新登录</p>
                </div>
              </div>
            </>
          ),
